...
def __main__():
	out_file = open('station_motion_all.html', 'w')
	out_file.write('<html>\n')
	out_file.write('<head>\n')
	...
	for station_num in range(1, 145, 1):
		...
		for row in motion_csv:
			...
			out_file.write('["' + '(A)' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v2 + ', ' + v3 + ', ' + v4 + ', "All"],\n')
			out_file.write('["' + '(S)' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v5 + ', ' + v6 + ', ' + v7 + ', "Same"],\n')
			if (station_num >= 144) and (count >= 24) :
				out_file.write('["' + '(D)' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v8 + ', ' + v9 + ', ' + v10 + ', "Diff"]\n')
			else:
				out_file.write('["' + '(D)' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v8 + ', ' + v9 + ', ' + v10 + ', "Diff"],\n')
			count = count + 1
	...
	out_file.write('\'{"colorOption":"5","xZoomedIn":true,"uniColorForNonSelected":false,"xZoomedDataMin":-4.099529190207156,"orderedByX":false,"yZoomedIn":true,"yAxisOption":"4","showTrails":false,"xLambda":1,"xZoomedDataMax":60,"yZoomedDataMax":3,"sizeOption":"2","yZoomedDataMin":-0.4212379892020292,"orderedByY":false,"dimensions":{"iconDimensions":["dim0"]},"playDuration":15000,"iconType":"BUBBLE","iconKeySettings":[],"duration":{"multiplier":1,"timeUnit":"D"},"nonSelectedAlpha":0.4,"time":"2013","xAxisOption":"3","yLambda":1}\';\n')
	#out_file.write('\'{"yAxisOption":"4","colorOption":"5","xAxisOption":"3","duration":{"multiplier":1,"timeUnit":"D"},"xZoomedIn":false,"time":"2013","yZoomedDataMax":2.94303361522201,"orderedByY":false,"showTrails":false,"xLambda":1,"xZoomedDataMax":170,"dimensions":{"iconDimensions":["dim0"]},"yZoomedIn":false,"sizeOption":"2","yZoomedDataMin":0,"orderedByX":false,"iconKeySettings":[],"playDuration":15000,"iconType":"BUBBLE","yLambda":1,"uniColorForNonSelected":false,"xZoomedDataMin":0.316666666666667,"nonSelectedAlpha":0.4}\';\n')

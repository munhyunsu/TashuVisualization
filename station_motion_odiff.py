...
def __main__():
	out_file = open('station_motion_odiff.html', 'w')
	out_file.write('<html>\n')
	out_file.write('<head>\n')
	...
	for station_num in range(1, 145, 1):
		...
		for row in motion_csv:
			...
			if (station_num >= 144) and (count >= 24):
				out_file.write('["' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v8 + ', ' + v9 + ', ' + v10 + ']\n')
			else:
				out_file.write('["' + station[station_num] + '", ' + 'new Date(' + str(int(v1[0:4])) + ', ' + str(int(v1[4:6])-1) + '), ' + v8 + ', ' + v9 + ', ' + v10 + '],\n')
			count = count + 1
	...
	out_file.write('\'{"yLambda":1,"yZoomedDataMax":4,"xZoomedDataMin":5.830929064657881,"duration":{"multiplier":1,"timeUnit":"D"},"nonSelectedAlpha":0.4,"time":"2013","orderedByX":false,"xAxisOption":"3","sizeOption":"2","yZoomedIn":true,"colorOption":"_UNIQUE_COLOR","iconKeySettings":[],"xZoomedIn":true,"xLambda":1,"orderedByY":false,"xZoomedDataMax":60,"yAxisOption":"4","uniColorForNonSelected":false,"yZoomedDataMin":0.3190844027223782,"showTrails":false,"dimensions":{"iconDimensions":["dim0"]},"playDuration":15000,"iconType":"BUBBLE"}\';\n')
